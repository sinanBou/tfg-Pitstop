    ...
    <div
      className={`group px-4 py-3 border rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all ${
        isLowStock 
          ...
          : 'bg-slate-100 dark:bg-neutral-950 border-slate-200 dark:border-neutral-850 hover:border-slate-300 dark:hover:border-neutral-800'
      }`}
    >
      <div className="flex items-start md:items-center gap-3 min-w-0 flex-1">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <p className="text-sm font-semibold text-slate-800 dark:text-neutral-200 truncate">{item.part.name}</p>
            <span className="text-[9px] bg-slate-200/60 dark:bg-white/5 border border-slate-300 dark:border-white/10 px-1.5 py-0.5 rounded text-slate-700 dark:text-neutral-400 font-mono font-bold">
              {item.part.manufacturer}
            </span>
          </div>
          {item.part.technicalSpecs && (
            <p className="text-[10px] text-slate-500 dark:text-neutral-500 font-medium truncate mt-0.5">
      ...
      <div className="flex flex-wrap items-center gap-3 shrink-0">
        ...
        <div className={`px-2.5 py-1 rounded-xl text-[10px] font-bold border flex items-center gap-1.5 ${
          ...
          {isLowStock && <span className="text-[8px] font-black uppercase text-amber-600/80 dark:text-amber-500/80">{t('partsTab.avisoSubText', { threshold: item.avisoThreshold })}</span>}
        </div>

        {/* Prices details */}
        <div className="px-2.5 py-1 bg-slate-100 dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-xl text-[10px] font-bold flex gap-3 text-slate-600 dark:text-neutral-400 font-mono">
          <div><span className="text-[8px] text-slate-400 dark:text-neutral-600 font-sans uppercase">{t('partsTab.costLabel')}</span> {formatCurrency(item.costPrice)}</div>
          <div className="border-l border-slate-300 dark:border-neutral-800 pl-3">
            <span className="text-[8px] text-slate-400 dark:text-neutral-600 font-sans uppercase">{t('partsTab.saleLabel')}</span> <span className="text-slate-900 dark:text-white font-bold">{formatCurrency(item.retailPrice)}</span>
          </div>
        </div>
