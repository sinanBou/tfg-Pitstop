...
export default function Registration() {
  ...
  const handleRegistrationSubmit = async (e: React.FormEvent) => {
    ...
    // Validaciones comunes de nombres
    ...
    // Validaciones específicas de taller
    if (role === 'workshop') {
      const wsData = workshopReg.formData;
      if (!wsData.companyName.trim()) { newErrors.companyName = 'El nombre del taller es obligatorio.'; isValid = false; }
      if (!wsData.cif.trim()) { newErrors.cif = 'El CIF del taller es obligatorio.'; isValid = false; }
      if (!wsData.phoneNumber.trim()) { newErrors.phoneNumber = 'El teléfono es obligatorio.'; isValid = false; }
      if (!wsData.address.trim()) { newErrors.address = 'La dirección del taller es obligatoria.'; isValid = false; }
    }

    // Validaciones específicas de cliente
    if (role === 'client') {
      const clientData = clientReg.formData;
      if (!clientData.nif.trim()) { newErrors.nif = 'El DNI/NIF es obligatorio.'; isValid = false; }
      if (!clientData.phoneNumber.trim()) { newErrors.phoneNumber = 'El teléfono es obligatorio.'; isValid = false; }
    }
  ...
  return (
    <div className="min-h-screen w-full flex flex-col justify-center items-center p-6 relative bg-zinc-950 font-sans selection:bg-red-500/30 selection:text-white overflow-y-auto">
              ...
              <form onSubmit={handleRegistrationSubmit} className="flex flex-col gap-6">
                ...
                {role === 'workshop' && (
                  <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <InputGroup label="Nombre del Dueño" name="firstname" value={workshopReg.formData.firstname} onChange={workshopReg.handleChange} error={errors.firstname} placeholder="Juan" />
                      <InputGroup label="Apellidos del Dueño" name="lastname" value={workshopReg.formData.lastname} onChange={workshopReg.handleChange} error={errors.lastname} placeholder="Pérez" />
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <InputGroup label="Nombre del Taller / Empresa" name="companyName" value={workshopReg.formData.companyName} onChange={workshopReg.handleChange} error={errors.companyName} placeholder="Ej. Talleres Motosport" />
                      <InputGroup label="CIF del Taller" name="cif" value={workshopReg.formData.cif} onChange={workshopReg.handleChange} error={errors.cif} placeholder="B12345678" />
                    </div>

                    <div className="grid grid-cols-1 gap-5">
                      <InputGroup label="Teléfono del Dueño" name="phoneNumber" value={workshopReg.formData.phoneNumber} onChange={workshopReg.handleChange} error={errors.phoneNumber} placeholder="Ej. 600123456" />
                    </div>
                ...
                <div className="border-t border-neutral-800/50 my-2 pt-6 flex flex-col gap-5 relative">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <InputGroup label="Confirmar Contraseña" name="confirmPassword" type="password" value={role === 'client' ? clientReg.formData.confirmPassword : workshopReg.formData.confirmPassword} onChange={role === 'client' ? clientReg.handleChange : workshopReg.handleChange} error={errors.confirmPassword} placeholder="Repite la contraseña" />
                  </div>
                </div>
