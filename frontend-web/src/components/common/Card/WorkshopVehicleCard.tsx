...
export const WorkshopVehicleCard: React.FC<WorkshopVehicleCardProps> = ({ vehicle }) => {
  ...
  const getThemeColors = (status: string) => {
    switch (status) {
      case 'COMPLETED':
        return {
          badge: 'bg-green-500/10 border-green-500/20 text-green-700 dark:text-green-400',
          ping: 'bg-green-500',
      ...
      case 'DELAYED':
        return {
          badge: 'bg-red-500/10 border-red-500/20 text-red-700 dark:text-red-400',
          ping: 'bg-red-500',
          gradient: 'from-blue-600 to-sky-400',
          border: 'border-blue-500/50 hover:border-blue-400 shadow-[0_0_15px_rgba(59,130,246,0.15)]',
  ...
  return (
    <div className={`bg-white dark:bg-gradient-to-br dark:from-neutral-900/80 dark:to-black border ${colors.border} p-5 rounded-2xl relative overflow-hidden group transition-all duration-300 flex flex-col justify-between shadow-sm hover:shadow-xl hover:-translate-y-1 w-[280px] h-[200px] shrink-0`}>
      {/* Icono de Fondo */}
      ...
      <div className="relative z-10 flex flex-col justify-between h-full gap-3">
        <div>
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-[11px] font-mono font-bold text-slate-700 dark:text-neutral-300 bg-slate-100 dark:bg-neutral-950/80 px-3 py-1 rounded-lg border border-slate-200 dark:border-neutral-800 shadow-inner tracking-wider">
              {vehicle.licensePlate}
            </span>
          </div>
          
          <h4 className="text-xl text-slate-900 dark:text-white font-black uppercase leading-tight flex items-center gap-1.5">
          ...
          <h5 className="text-xs text-slate-500 dark:text-neutral-400 font-bold uppercase truncate mt-0.5">{vehicle.model}</h5>
        </div>
        
        {/* Barra de Progreso */}
        <div className="mt-auto">
          <div className="flex justify-between items-center mb-1.5">
            <span className="text-[8px] font-black uppercase tracking-widest text-slate-500 dark:text-neutral-500 font-bold">{t('workshopVehicleCard.repairProgress')}</span>
          </div>
          <div className="h-2 w-full bg-slate-100 dark:bg-neutral-950 rounded-full overflow-hidden shadow-inner border border-slate-200 dark:border-neutral-800/50">
            <div 
