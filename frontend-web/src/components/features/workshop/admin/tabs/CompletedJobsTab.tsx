 ...
 *  COMPLETED JOBS TAB
 *  Two-step workflow:
 *  1. IN_PROGRESS (all tasks done) → "Completar Trabajo" → sets COMPLETED
 *  2. COMPLETED → "Vehículo Recogido" → sets PICKED_UP (removed from panel)
 * ───────────────────────────────────────────── */
...
const JobCard: React.FC<JobCardProps> = ({ job, phase, isProcessing, onAction }) => {
  ...
  return (
    <div
      className={`bg-neutral-900/40 border rounded-[2rem] p-6 relative overflow-hidden group transition-all duration-300 hover:shadow-[0_0_40px_rgba(34,197,94,0.1)] ${
        isApprove
    ...
    >
      ...
      <div className="relative z-10 space-y-4">
        ...
        <button
          ...
          className={`w-full py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 active:scale-[0.98] disabled:bg-neutral-800 disabled:text-neutral-600 disabled:shadow-none ${
            isApprove
              ? 'bg-amber-600 hover:bg-amber-500 text-white shadow-[0_0_20px_rgba(245,158,11,0.3)] hover:shadow-[0_0_30px_rgba(245,158,11,0.5)]'
